...
class App:
    ...
    def __init__(self):
        root = tk.Tk()
        root.title('Strawberry Educations Demo')
        ...
        button =tk.Button(right, text="Browse",command=self.load_file, width=8)
        ...
        button.pack()
    def load_file(self):
         self.fname = askopenfilename(filetypes=(("All files", "*.*"),("Template files", "*.tplate"),
                                           ("HTML files", "*.html;*.htm"),
                                            ))
         insert(self.fname)
